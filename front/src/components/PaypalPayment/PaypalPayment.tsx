...
export default function PaypalPayment({ onSuccess }: PaypalPaymentProps) {
  ...
  const onApprove = async (data: any, actions: any) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/successPayments`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      // Call onSuccess after successful payment
      if (onSuccess) {
        onSuccess();
        Swal.fire({
          icon: "success",
