...
const useFetchUsers = () => {
  ...
  useEffect(() => {
    ...
    // Si le token n'est pas trouvé et que le chemin actuel n'est pas '/auth/admin/login' ou '/auth/register' ou '/connection' ou '/confirm-email' alors afficher une erreur
    if (!token && currentPath !== '/auth/admin/login' && currentPath !== '/auth/register' && currentPath !== '/connection' && currentPath !== '/confirm-email') {
      console.error("Token not found");
      localStorage.setItem("showSwal", "true");
      // Rediriger vers la page de connexion au lieu de /auth/admin/login
