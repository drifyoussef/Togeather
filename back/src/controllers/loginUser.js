  ...
  try {
    ...
    if (userFind) {
      ...
      if (userFind.isBanned) {
        ...
        const token = jwt.sign({ _id: userFind._id }, privateKey, {
          algorithm: "RS256",
        });
      ...
      if (userFind.isEmailConfirmed === false) {
        ...
        res
          .status(401)
          .json({ message: "Email non confirmé", emailNotConfirmed: true });
        return;
      }
      ...
      if (same) {
        ...
        console.log(
          req.session.userId,
          "ID DE L'UTILISATEUR LOGIN CONTROLLER REQ SESSION USERID"
        );
        const privateKey = fs.readFileSync(
          path.join(appRoot.path, "private.key")
        );
